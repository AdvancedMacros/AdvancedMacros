...
local changeLog = {
  ...
  "&bFeatures:",
  " - Added getter for &dItemStack&f in &aopenInventory", --check
  " - Added getter for &dEntity&f in &egetEntity", --check
  " - Added &bgetNBT&f, &bgetPlayerNBT&f and &bgetEntityNBT&f",
  "   getNBT(  entityID or playerName  ) -> nbt table",
  "   getNBT( number: entity id ) -> nbt table  (alias)",
  "   getNBT( string: playerName ) -> nbt table  (alias)",
  " - '&6runners accept bytecode&f'",
  " - '&ahttpQuick&f for simpler http requests'", --needs more notes
 --require 'json'? luajava Json or lua only?
  "   - buildHTTP( request )",
  "   - httpQuick( url, request )",
  " - &eFileSystem&f read file mode now has",
  "    .&breadBytes&f() and .&bskipBytes&v()",
  " - '&6inv.setRecipe for recipe book behavior&f'",
  "   - &bgetRecipe&f includes recipe id",
  "   - &aopenInventory&f contorls now has function &bsetRecipe",
  " - added support for &ewav&f, &emod&f, &es3m&f, and &exm&f &daudio files",
  "   - sample audio files included",
  "     - &dMortimers_chipdisko.mod",
  "     - &dlowhealth_AoL.ogg",
  "     - &dlowhealth_LA.ogg",
  "     - &d&dlowhealth_LoZ.ogg",
  "     - &d&dlowhealth_LttP.ogg",
  "     - &dlowhealth_OoT.ogg",
  "     - &dlowhealth_Oracle.ogg",
  "     - &dmessage.ogg",
  "     - &dmilky.xm",
  "     - &d&dpling.ogg",
  "     - &dpop.wav",
  ...
  "     - &dtux-leveldone.mod",
  "     - &dwaiting-loop.ogg",
  " - '&binv.drop&f takes stack param as &eboolean&f'",
  " - 'setTrade, checkTrade and selectedTrade for Villager GUI'",
  "   - isEnabled, maxUses, uses, selectedTrade, checkTrade, setTrade",
  " - '&binv.quick&f support for table and vararg' - can specify multiple slots to click in order",
  " - 'implement &blogdel&f' adds ability to remove lines from chat",
  "   &blogdel&f( int: chatline id )",
  "   &blogdel&f( table: chatline ids )",
  "   &blogdel&f( ...: log args ) -> id",
  " - added &bgetMinecraft&f(), returns luajava instance of &dMinecraft class",
  " - &cadded &b_env&c() returns AdvancedMacos:_env() &7seems this one might be broken", --?? doesn't exist in AM class
  " - http &bgetHeaderFields&f from response",
  " - &bgetLoadedPlayers&f accepts optional boolean to include NPCs",
  " - &bentityRayTrace&f added, takes table args, everything is optional",
  "     &bentityRayTrace&f{",
  "       &6yaw&f    = &7number",
  "       &6pitch&f  = &7number",
  "       &6from&f   = &e{&7x, y, z&e}   &fAlias &6src&f, &6source", --lines up
  "       &6fluids&f = &7boolean&f     &fAlias &6includeFluids&f, &6fluid&f,",
  "                                  &6liquids&f, &6liquid&f, &6stopOnLiquid", --this lines up
  "       &6reach&f   = &7number&f      &fAlias &6dist&f, &6distance&f, &6range&f",
  "     }",
  "     &6yaw&f & &6pitch&f can be replaced with",
  "       &6to&f = &e{&7x, y, z &e} &fAlias &6dest&f, &6destination&f, &6target&f",
  "   - result from &bentityRayTrace&f() includes a &dfunction&f that returns",
  "     the entity info if needed",
  " - &bnewAABB&f( &7...&f ) creates a new &eAxisAlignedBoundingBox",
  "   args consume 2 vectors, either as tables with 3 numbers, or in the unpacked form (or some mix)",
  " - &bnewClass(&6string&f:&7className&f, <&dclass&7:extends>)&f which requires significantly less boilerplate",
  "   then &aadvancedMacros&f.&binherit&f(...) See the wiki",
  " - added &bisClass( &7something&f ) for type checking",
  "   - for more specific type checking use &aadvancedmacros.utils.&btypeMatches&f( x, {\"&6class:com.theincgi.Foo&f\"})",
  " - added &aadvancedMacros.utils&f which contains lots functions which handle common opperations",
  "   &eeven less boilerplate!",
  " - &7kwargs function lets you type check named arguments, provide defaults, aliases and type checks all at once",
  "   &7you can also see if an alias was used if you want to handle it differently, see wiki",
  "&aBug Fixes:",
  " - Fixed &7RMENU&f + &7A&f triggering text selection",
  " - 'hotfix JavaConstructor all setAccessible'", --
  " - 'fix to Lua coercion of sound clips'", --
  " - 'fix setText not updating draw' in ingame text editor",
  " - 'fix say function for Baritone'",
  " - 'fix NPE in HTTP getContentType'",
  --" - 'fix false-positive bytecode detection' ingame editor", added bytecode detection?
  " - 'fix charAt without empty check' related to player list",
  " - 'fix dim change event leaking across servers'",
  " - 'RAFFuncs, fix string corruption'",
  " - 'fix issues with CodecIBXM' sound related, http://www.paulscode.com",
  " - 'FileSystem Write and WriteLine, fix string corruption'",
  " - 'NBTTagByteArray is read as LuaString'",
  " - 'inventory &bdrop&f and hotbar swap (&bhotSwap&f)' &aopenInventory&f controls",
  " - 'fix nullptr exception in &bgetPlayerList&f for aMToXIC'", --quietly errors and returns empty table
  " - &bplaySound&f and &bgetSound&f both have &bgetClip&f() which returns the java object for the sound",
  " - 'StringSerialization fix string corruption' - Lua 5.3 feature, string pack/unpack",
  " - 'filesystem readAll, fix string corruption'",
  " - 'FileSystem Write and WriteLine, fix string corruption'",
  " - 'http write and writeLine, fix string corruption'",
  " - 'luaj getField fix by wagyourtail' - luajava",
  " - 'http getContent returns now valid LuaString'",
  " - 'added missing GetNBT class'", --merge?
  " - 'porting bugfixes from upper versions'", --more merge?
  " - 'fix proxy calls on non lua threads'",
  " - 'exclude bots in getLoadedPlayers'",
  " - 'bind back chat to minecraft thread'",
  " - 'fix chat timing out by filter",
  " - 'mute spam from missing docs'",
  " - 'fix leaking threads'",
  " - openInventory dragClick",
  "&eAdditional Notes:",
  " - Now using mixins instead of ASM for max chat lines & custom chat click events",
  " - entities returned from functions as &aLuaTable&f now include the entity &bid",
  " - scripts are utf-8 now, typing in unicode",
  " - Forge version updated",
  " - 'bind ticked events to ClientTick'",
  " - 'read scripts as UTF-8 encoded files'",
  " - indexing nil in any &aLuaTable&f returns nil",
  " - 'prototype of 'luajava.wrap''",
  " - 'efficient getContent for httpRequest'",
  " - 'change getPlayerList' -modified",
  " - 'optimize populatePlayerList'",
  " - 'mute debug chat filter prints' &7- less console spam",
  " - updated Forge licenses",
  " - getPlayer().team is &afalse&f instead of '&6none&f' when not on a team now", --player.?
  " - axis aligned bounding box (aabb) for a block is FALSE if air",
  " - &bgetEntity(&7id, <boolean>&f)&f, opt second boolean returns luajava of entity",
  
  "&7"..DIVIDER,
  
  "&b&BChange Log: &7version 7.10.0",
  "&bFeatures:",
